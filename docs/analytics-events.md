...
어드민의 루핏 프로모션 블록은 선택 기간·세그먼트의 사람 트래픽 중
`promotionId=loofit-ios`인 이벤트만 대상으로 총 노출, 총 클릭, 이벤트 CTR을
표시한다. 위치 표는 같은 `source`·`variant` 조합으로 묶고 클릭 수와 함께 노출
수·이벤트 CTR을 보여준다. 이벤트 CTR은 클릭 이벤트 수를 노출 이벤트 수로 나눈
값이며, 같은 노출 뒤 반복 클릭이 수집될 수 있어 100%를 넘을 수 있다. 사용자·세션
단위의 고유 클릭률로 해석하지 않는다.
...
깊이 분석용). 광고 문의 슬롯은 노출(impression)을 아직 측정하지 않으므로 슬롯
지표는 문의 클릭(`contact_open`) 기준이다.
